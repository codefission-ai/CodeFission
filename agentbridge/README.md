...
from agentbridge import SessionConfig, ProviderType
...
config = SessionConfig(
    ...
    permission_mode="bypassPermissions",   # Claude: auto-approve tools
    sandbox_mode="workspace-write",        # Codex: sandbox level
...
agentbridge/
  ...
  types.py              # ProviderType enum, SessionConfig dataclass
...
111 tests covering events, types, pricing, context transfer, session manager (switch/apply/build_config), adapter command building, adapter streaming with mocked subprocess output, and end-to-end cross-provider round-trips.
